...
import com.example.demoMakeBoard.dto.PasswordForm;
...
public class HomeController {
  ...
  public String postMemberAdd(@Valid @ModelAttribute("member")MemberForm memberForm, BindingResult bindingResult){
    if (memberForm.getPassword() == null || memberForm.getPassword().trim().length() < 8){
      bindingResult.rejectValue("password", "NotBlack", "패스워드를 8글자 이상 입력하세요");
    }
    if(!memberForm.getPassword().equals(memberForm.getPasswordConfirm())){
      bindingResult.rejectValue("password", "MissMatch", "입력하신 패스워닥 다릅니다.");
    }
  ...
  public String postPassword(@ModelAttribute("password")PasswordForm passwordForm, BindingResult bindingResult,
                             @AuthenticationPrincipal MemberUserDetails userDetails){
    if(!memberService.checkPassword(userDetails.getMemberId(), passwordForm.getOld()))
    { bindingResult.rejectValue("old", "MissMatch", "비멀번호가 잘못 되었다.");
    }
